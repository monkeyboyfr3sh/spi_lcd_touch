...
static void update_plot(void)
{
    ...
    lv_chart_set_next_value(chart, ser1, (int)new_x * graph_sensitivity);
    lv_chart_set_next_value(chart, ser2, (int)new_y * graph_sensitivity);
...
static void slider_event_cb(lv_event_t * e)
{
    ...
    set_backlight_brightness( ((float)intense/255.0) );
}
...
void create_lvgl_ui(display_mode_t display_mode)
{
    ...
    const float g_scale = 1.1; // # of g to show
