...
const float graph_sensativity = 100.0;
...
static void update_obect(lv_obj_t *object_x, lv_obj_t *object_y, lv_obj_t *object_z, float new_x, float new_y, float new_z)
{
    ...
    lv_bar_set_value(bar_x, new_x*graph_sensativity, LV_ANIM_OFF);
    lv_bar_set_value(bar_y, new_y*graph_sensativity, LV_ANIM_OFF);
    lv_bar_set_value(bar_z, new_z*graph_sensativity, LV_ANIM_OFF);
}
...
void create_lvgl_ui(void)
{
    ...
    const int bar_range = (acceleration * g_scale) * graph_sensativity; // Applying graph sensativity scale
